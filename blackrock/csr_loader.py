...
def load_all_csrs(
    csr_root: Optional[str] = None, schema_path: Optional[str] = None
) -> Dict[str, CSRDef]:
    """Load and validate all CSR YAML files in csr_root; returns dict name -> CSRDef."""
    if csr_root is None:
    ...
    result: Dict[str, CSRDef] = {}
    for file in os.listdir(csr_root):
        if file.endswith(".yml") or file.endswith(".yaml"):
            csr_path = os.path.join(csr_root, file)
            validate_yaml_schema(csr_path, schema_path)
            data = load_yaml(csr_path)
            name = data["name"]
            result[name] = CSRDef(
